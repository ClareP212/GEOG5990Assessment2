...
def update(frame_number):
    ...
    for j in gen_function(): 
        ...
        if bacteria[i].height == 0:
            carry_on = False
#attach final x y location to bacteria_location            
    bacteria_location.append([bacteria[i].y,bacteria[i].x])
    print("bacteria height = " + str(bacteria[i].height))
    print(i)
...
def run():
    ...
    """
    animation = matplotlib.animation.FuncAnimation(fig, update, repeat=False, frames = num_of_bacteria, interval=10)
    canvas.draw()
